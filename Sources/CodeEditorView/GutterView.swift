...
private let lineNumberColour = OSColor(
    red: 0.5,
    green: 0.5,
    blue: 0.5,
    alpha: 0.5
...
extension GutterView {
    ...
    var optTextLayoutManager: NSTextLayoutManager? {
        textView?.optTextLayoutManager
    }
    var optTextContentStorage: NSTextContentStorage? {
        textView?.optTextContentStorage
    }
    var optTextContainer: NSTextContainer? { textView?.optTextContainer }
    var optLineMap: LineMap<LineInfo>? {
        (codeStorage.delegate as? CodeStorageDelegate)?.lineMap
    }
    ...
    func invalidateGutter(for charRange: NSRange? = nil) {
        ...
        guard let textLayoutManager = optTextLayoutManager,
            ...
            let viewPortRange = textLayoutManager.textViewportLayoutController
                .viewportRange,
            let viewPortRangeExt = textContentStorage.range(for: viewPortRange)
                .shifted(endBy: 1),
        ...
        let extendedCharRange = string.paragraphRange(
            for: charRangeInViewPort.clamped(to: string.length)
        )

        if let textRange = textContentStorage.textRange(for: extendedCharRange),
            let (y: y, height: height) =
                textLayoutManager.textLayoutFragmentExtent(for: textRange),
            height > 0
        {
        ...
        if charRange.max == string.length,
            let endLocation = textContentStorage.textLocation(
                for: charRange.max
            ),
    ...
    override func draw(_ rect: CGRect) {
        ...
        let viewPortBounds = textLayoutManager.textViewportLayoutController
            .viewportBounds
        textLayoutManager.ensureLayout(for: viewPortBounds)

        let desc = OSFont.systemFont(ofSize: theme.fontSize).fontDescriptor
            .addingAttributes(
                [
                    OSFontDescriptor.AttributeName.featureSettings:
                        [
                            [
                                fontDescriptorFeatureIdentifier:
                                    kNumberSpacingType,
                                fontDescriptorTypeIdentifier:
                            ...
                            [
                                ...
                                fontDescriptorTypeIdentifier:
                                    kStylisticAltOneOnSelector,  // alt 6 and 9
                            ],
                            [
                                fontDescriptorFeatureIdentifier:
                                    kStylisticAlternativesType,
                                fontDescriptorTypeIdentifier:
                                    kStylisticAltTwoOnSelector,  // alt 4
                            ],
                        ]
                ]
            )
        ...
        #elseif os(macOS)
            let font =
                OSFont(descriptor: desc, size: 0)
                ?? OSFont.systemFont(ofSize: 0)
        #endif
        ...
        if !isMinimapGutter {
            ...
            for line in lineRange {  // NB: These are zero-based line numbers
                ...
                guard
                    let lineStartLocation = textContentStorage.textLocation(
                        for: lineMap.lines[line].range.location
                    ),
                    let textLayoutFragment =
                        textLayoutManager.textLayoutFragment(
                            for: lineStartLocation
                        )
                else { continue }

                let gutterRect = gutterRectForLineNumbersFrom(
                    textRect:
                        textLayoutFragment
                        .layoutFragmentFrameWithoutExtraLineFragment
                )
                let attributes =
                    selectedLines.contains(line)
                    ? textAttributesSelected : textAttributesDefault
                if gutterRect.intersects(rect) {
                    ("\(line + 1)" as NSString).draw(
                        in: gutterRect,
                    ...
                    .layoutFragmentFrameExtraLineFragment
            {
                ...
                let attributes =
                    selectedLines.contains(lineRange.endIndex - 1)
                    ? textAttributesSelected : textAttributesDefault
                if gutterRect.intersects(rect) {
                    ("\(lineMap.lines.count)" as NSString).draw(
                        in: gutterRect,
                        withAttributes: attributes
                    )
                }
                ...
                let gutterRect = gutterRectForLineNumbersFrom(
                    textRect: firstTextRect
                )
                if gutterRect.intersects(rect) {
                    ("\(1)" as NSString).draw(
                        in: gutterRect,
                        withAttributes: textAttributesSelected
                    )
                }
            }
        }
...
extension GutterView {
    ...
    private func gutterRectFrom(y: CGFloat, height: CGFloat) -> CGRect {
        return CGRect(
            origin: CGPoint(
                x: 0,
                y: y + (textView?.textContainerOrigin.y ?? 0)
            ),
    ...
    private func gutterRectForLineNumbersFrom(textRect: CGRect) -> CGRect {
        let gutterRect = gutterRectFrom(
            y: textRect.minY,
            height: textRect.height
        )
        return CGRect(
            x: gutterRect.origin.x + gutterRect.size.width * 1 / 7,
    ...
    private func textRectFrom(gutterRect: CGRect) -> CGRect {
        ...
        return CGRect(
            origin: CGPoint(
                x: frame.size.width,
                y: gutterRect.origin.y - (textView?.textContainerOrigin.y ?? 0)
            ),
            size: CGSize(
                width: containerWidth - frame.size.width,
                height: gutterRect.size.height
            )
        )
    }
