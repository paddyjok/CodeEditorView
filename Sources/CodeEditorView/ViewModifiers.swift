...
private struct RectWithRoundedCornersOnTheLeft: Shape {
    ...
    func path(in rect: CGRect) -> Path {
        ...
        let minYCorner = rect.minY + cornerRadius
        let maxYCorner = rect.maxY - cornerRadius
        ...
        path.addArc(
            center: CGPoint(x: minXCorner, y: maxYCorner),
            radius: cornerRadius,
        ...
        path.addArc(
            center: CGPoint(x: minXCorner, y: minYCorner),
            radius: cornerRadius,
            startAngle: Angle(degrees: 90),
