...
extension NSTextLayoutFragment {
    ...
    var layoutFragmentFrameWithoutExtraLineFragment: CGRect {
        ...
        if let lastTextLineFragment = textLineFragments.last,
...
extension NSTextLayoutManager {
    ...
    ///
    func textLayoutFragmentExtent(for textRange: NSTextRange) -> (
        ...
        if location.compare(documentRange.endLocation) == .orderedSame {  // Start of range == end of the document
            ...
            if let lastLocation = textContentManager?.location(
                textRange.endLocation,
                offsetBy: -1
            ),
                let lastTextLayoutFragment = textLayoutFragment(
                    for: lastLocation
                ),
                let lastTextLineFragment = lastTextLayoutFragment
            ...
            {  // trailing newline at the end of the document
                ...
                return (
                    y: lastTextLayoutFragment.layoutFragmentFrame.minY
                ...
                if let endLocation = textContentManager?.location(
                    textRange.endLocation,
                    offsetBy: -1
                ),
                    let endFrame = textLayoutFragment(for: endLocation)?
                        .layoutFragmentFrame
                {
            ...
            let endLocation =
                if textRange.isEmpty { nil as NSTextLocation? } else {
                    textContentManager?.location(
                        textRange.endLocation,
                        offsetBy: -1
                    )
                }
            let startFragment = textLayoutFragment(for: location)
            let startFrame = startFragment?.layoutFragmentFrame
            let endFragment =
                if let endLocation { textLayoutFragment(for: endLocation) } else
                { nil as NSTextLayoutFragment? }
            let endFrame = endFragment?.layoutFragmentFrameAdjustedKludge

            switch (startFrame, endFrame) {
            case (nil, nil): return nil
            case (.some(let startFrame), nil):
                return (y: startFrame.minY, height: startFrame.height)
            case (nil, .some(let endFrame)):
                return (y: endFrame.minY, height: endFrame.height)
            ...
            case (.some(let startFrame), .some(let endFrame)):
                if startFrame.minY < endFrame.minY {
                    return (
                        y: startFrame.minY,
                        height: endFrame.maxY - startFrame.minY
                    )
                } else {
                    return (
                        y: endFrame.minY,
                        height: startFrame.maxY - endFrame.minY
                    )
                }
            }
        }
        ...
        -> NSTextLocation?
    {
        ...
        enumerateTextLayoutFragments(from: textRange.location, options: options)
        { textLayoutFragment in
            textLayoutFragment.rangeInElement.location.compare(
                textRange.endLocation
            ) == .orderedAscending
    ...
    func enumerateRenderingAttributes(
        ...
        reverse: Bool,
        using block: (
            NSTextLayoutManager, [NSAttributedString.Key: Any], NSTextRange
        ) -> Void
    ) {
        if !reverse {
            ...
            enumerateRenderingAttributes(
                ...
                reverse: false
            ) { textLayoutManager, attributes, attributeRange in

                if let clippedRange = attributeRange.intersection(textRange) {
                    block(textLayoutManager, attributes, clippedRange)
                }
                return attributeRange.endLocation.compare(textRange.endLocation)
                    == .orderedAscending
            }
            ...
            enumerateRenderingAttributes(
                ...
                return attributeRange.location.compare(textRange.location)
                    == .orderedDescending
            }
    ...
    ///
    typealias AttributeRun = (
        attributes: [NSAttributedString.Key: Any], textRange: NSTextRange
    )
    ...
    func renderingAttributes(in textRange: NSTextRange) -> [AttributeRun] {
        ...
        enumerateRenderingAttributes(in: textRange, reverse: false) {
            attributes.append((attributes: $1, textRange: $2))
        }
        return attributes
    }
}
...
extension NSTextLayoutManager {
    ...
    func setSafeRenderingAttributesValidator(
        with codeViewDelegate: CodeViewDelegate,
        ...
        -> NSKeyValueObservation?
    {
        ...
        self.renderingAttributesValidator = {
            textLayoutManager,
            textLayoutFragment in
            ...
            if let textContentStorage = textLayoutManager.textContentManager
                as? NSTextContentStorage,
                let codeStorageDelegate = textContentStorage.textStorage?
