...
private let logger = Logger(
    subsystem: "org.justtesting.CodeEditorView",
    category: "CodeView"
)
    ...
    final class CodeView: UITextView {
        ...
        var theme: Theme = .defaultLight {
            didSet {
                ...
                documentVisibleBox?.backgroundColor = theme.textColour
            ...
            didSet {
                if let codeStorage = optCodeStorage,
                    oldValue.name != language.name
                        || (oldValue.languageService != nil)
                            != (language.languageService != nil)
                {
                    Task { @MainActor in
                        try await codeStorageDelegate.change(
                            language: language,
                            for: codeStorage
                        )
        ...
        init(
            ...
            let textContentStorage = CodeContentStorage()
...
//                        codeStorage.setHighlightingAttributes(for: fragmentRange, in: textLayoutManager)
            ...
            let currentLineHighlightView = CodeBackgroundHighlightView(
                color: theme.currentLineColour
            )
            ...
            textDidChangeObserver = NotificationCenter.default.addObserver(
                ...
                self?.invalidateMessageViews(
                    withIDs: self!.codeStorageDelegate.lastInvalidatedMessageIDs
                )
    ...
    final class CodeViewDelegate: NSObject, UITextViewDelegate {
        ...
        func textViewDidChangeSelection(_ textView: UITextView) {
            ...
            codeView.updateBackgroundFor(
                oldSelection: oldSelectedRange,
                newSelection: codeView.selectedRange
            )
    ...
    final class CodeView: NSTextView {
        ...
        fileprivate var codeStorageDelegate: CodeStorageDelegate
        fileprivate let minimapTextLayoutManagerDelegate =
            MinimapTextLayoutManagerDelegate()
        ...
        var theme: Theme = .defaultLight {
            didSet {
                ...
                selectedTextAttributes = [
                    .backgroundColor: theme.selectionColour
                ]
                (textStorage as? CodeStorage)?.theme = theme
                gutterView?.theme = theme
                currentLineHighlightView?.color = theme.currentLineColour
                minimapView?.backgroundColor = theme.backgroundColour
                minimapGutterView?.theme = theme
                documentVisibleBox?.fillColor = theme.textColour
                    .withAlphaComponent(0.1)
            }
        }
                ...
                if oldValue != language {
                    ...
                    Task { @MainActor in
                        do {
                            ...
                            logger.trace(
                                "Failed to change language from \(oldValue.name) to \(self.language.name): \(error.localizedDescription)"
                            )
                        }
        ...
        init(
            ...
            textLayoutManager.setSafeRenderingAttributesValidator(
                with: codeViewDelegate
            ...
            let minimapGutterView = GutterView(
                frame: CGRect.zero,
            ...
            minimapView.textLayoutManager?.setSafeRenderingAttributesValidator(
                with:
                    minimapCodeViewDelegate
            ) {
                (
                    minimapLayoutManager,
                    layoutFragment
                ) in
                guard
                    let textContentStorage = minimapLayoutManager
                        .textContentManager as? NSTextContentStorage
                else { return }
                codeStorage.setHighlightingAttributes(
                    for: textContentStorage.range(
            ...
            minimapView.textLayoutManager?.delegate =
                minimapTextLayoutManagerDelegate

            let font = theme.font
            minimapView.font = OSFont(
                name: font.fontName,
                size: font.pointSize / minimapRatio
            )!
            ...
            documentVisibleBox.fillColor = theme.textColour.withAlphaComponent(
                0.1
            )
            ...
            Task { @MainActor in
                minimapView.textLayoutManager?.invalidateLayout(
                    for: minimapView.textLayoutManager!.documentRange
                )
            }

            // We need to re-tile the subviews whenever the frame of the text view changes.
            frameChangedNotificationObserver = NotificationCenter.default
                .addObserver(
            ...
            // We need to check whether we need to look up completions or cancel a running completion process after every text
            // change. We also need to invalidate the views of all in the meantime invalidated message views.
            didChangeNotificationObserver = NotificationCenter.default
                .addObserver(
                    ...
                    self?.considerCompletionFor(
                        range: self!.rangeForUserCompletion
                    )
                    self?.invalidateMessageViews(
                        withIDs: self!.codeStorageDelegate
                            .lastInvalidatedMessageIDs
                    )
                }
            ...
            Task {
                do {
                    ...
                    logger.trace(
                        "Failed to start language service for \(language.name): \(error.localizedDescription)"
                    )
                }
            }
        }
        ...
        func startLanguageService() async throws {
            ...
            if let languageService = codeStorageDelegate.languageService {
                ...
                try await languageService.openDocument(
                    with: textStorage.string,
                    locationService: codeStorageDelegate
                        .lineMapLocationConverter
                )
        ...
        deinit {
            if let observer = frameChangedNotificationObserver {
                NotificationCenter.default.removeObserver(observer)
            }
            if let observer = didChangeNotificationObserver {
                NotificationCenter.default.removeObserver(observer)
            }
        }
        ...
        override func setSelectedRanges(
            ...
            if oldSelectedRanges != selectedRanges {
                ...
                if let insertionPoint {
                    minimapView?.setSelectedRange(
                        NSRange(location: insertionPoint, length: 0)
                    )
                }

                updateBackgroundFor(
                    oldSelection: combinedRanges(ranges: oldSelectedRanges),
    ...
    final class CodeViewDelegate: NSObject, NSTextViewDelegate {
        ...
        {
            guard
                let codeStorageDelegeate = textView.textStorage?.delegate
                ...
                selectionRange.length == 0
            {
                ...
                let insertionPointWithoutCompletion =
                    selectionRange.location
                    - codeStorageDelegeate.tokenCompletionCharacters
                codeStorageDelegeate.tokenCompletionCharacters = 0
                return [
                    NSRange(
                        location: insertionPointWithoutCompletion,
                        length: 0
                    ) as NSValue
                ]
...
extension CodeView {
    ...
    /// (highlighting) views.
    ///
    func updateBackgroundFor(
        oldSelection oldRange: NSRange,
        newSelection newRange: NSRange
    ) {
        guard let textContentStorage = optTextContentStorage else { return }

        let lineOfInsertionPoint = insertionPoint.flatMap {
            optLineMap?.lineOf(index: $0)
        }
        ...
        if lineOfInsertionPoint != oldLastLineOfInsertionPoint {
            ...
            if let textLocation = textContentStorage.textLocation(
                for: newRange.location
            ) {
    ...
    func updateCurrentLineHighlight(for location: NSTextLocation) {
        ...
        if let fragmentFrame = textLayoutManager.textLayoutFragment(
            for: location
        )?.layoutFragmentFrameWithoutExtraLineFragment,
            let highlightRect = lineBackgroundRect(
        ...
        if let previousLocation = optTextContentStorage?.location(
            location,
            offsetBy: -1
        ),
            let fragmentFrame = textLayoutManager.textLayoutFragment(
                for: previousLocation
            )?.layoutFragmentFrameExtraLineFragment,
            let highlightRect = lineBackgroundRect(
                y: fragmentFrame.minY,
                height: fragmentFrame.height
            )
    ...
    func updateMessageLineHighlights() {
        ...
        for messageView in messageViews {
            ...
            if let telescopeCharacterIndex = messageView.value
                .characterIndexTelescope,
                !messageView.value.invalidated  // No telesopes for invalidates messages
            {

                if let startLocation = optTextContentStorage?.textLocation(
                    for: messageView.value.characterIndex
                ),
                    let endLocation = optTextContentStorage?.textLocation(
                        for: telescopeCharacterIndex
                    ),
                    let textRange = NSTextRange(
                        location: startLocation,
                        end: endLocation
                    ),
                    let extent = optTextLayoutManager?.textLayoutFragmentExtent(
                        for: textRange
                    ),
                    let highlightRect = lineBackgroundRect(
                        y: extent.y,
                ...
                if let textLocation = optTextContentStorage?.textLocation(
                    for: messageView.value.characterIndex
                ),
                    let fragmentFrame = optTextLayoutManager?
                        .textLayoutFragment(for: textLocation)?
                        .layoutFragmentFrameWithoutExtraLineFragment,
                    let highlightRect = lineBackgroundRect(
                        y: fragmentFrame.minY,
                        height: fragmentFrame.height
    ...
    @MainActor
    private func tile() {
        guard let codeContainer = optTextContainer as? CodeContainer else {
            return
        }
        ...
        if gutterView?.frame.size != gutterSize {
            gutterView?.frame = CGRect(origin: .zero, size: gutterSize)
        }
        ...
        let widthWithoutGutters = visibleWidth - gutterWithPadding
        let compositeFontWidth = fontWidth
        let numberOfCharacters = widthWithoutGutters / compositeFontWidth
        let codeViewWidth = visibleWidth

        #if os(iOS) || os(visionOS)
            showsHorizontalScrollIndicator = !viewLayout.wrapText
            if viewLayout.wrapText && frame.size.width != visibleWidth {
            ...
            if !isHorizontallyResizable && frame.size.width != visibleWidth {
                frame.size.width = visibleWidth
            }  // don't update frames in vain
        #endif
        ...
        let codeContainerWidth =
            if viewLayout.wrapText { codeViewWidth - gutterWidth } else {
                CGFloat.greatestFiniteMagnitude
            }
        if codeContainer.size.width != codeContainerWidth {
            codeContainer.size = CGSize(
    ...
    fileprivate func layoutMessageView(identifiedBy id: UUID) {
        ...
        if messageBundle.geometry == nil {
            ...
            guard
                let startLocation = textContentManager.textLocation(
                    for: messageBundle.characterIndex
                ),
                let textLayoutFragment = textLayoutManager.textLayoutFragment(
                    for: startLocation
                ),
                let firstLineFragmentRect = textLayoutFragment.textLineFragments
                    .first?.typographicBounds
            else { return }

            // Compute the message view geometry from the text layout information
            let geometry = MessageView.Geometry(
                lineWidth: messageBundle.lineFragementRect.width
                    - firstLineFragmentRect.maxX,
                lineHeight: firstLineFragmentRect.height,
                popupWidth: (codeContainer.size.width
                    - MessageView.popupRightSideOffset) * 0.75,
            ...
            if messageBundle.view.superview == nil {
                ...
                let topOffset =
                    textContainerOrigin.y + messageBundle.lineFragementRect.minY
                let topAnchorConstraint = messageBundle.view.topAnchor
                    .constraint(
                        equalTo: self.topAnchor,
                        constant: topOffset
                    )
                let leftOffset =
                    textContainerOrigin.x + messageBundle.lineFragementRect.maxX
                let rightAnchorConstraint = messageBundle.view.rightAnchor
                    .constraint(
                ...
                NSLayoutConstraint.activate([
                    topAnchorConstraint, rightAnchorConstraint,
                ])
                ...
                if let currentLineHighlightView {
                    insertSubview(
                        messageBundle.backgroundView,
                        aboveSubview: currentLineHighlightView
                    )
                }
    ...
    func report(message: TextLocated<Message>) {
        guard let messageBundle = codeStorageDelegate.add(message: message)
        else { return }

        updateMessageView(
            for: messageBundle,
            at: message.location.zeroBasedLine
        )
    }
    ...
    func retract(message: Message) {
        guard
            let (messageBundle, line) = codeStorageDelegate.remove(
                message: message
            )
        else { return }
    ...
    private func updateMessageView(
        for messageBundle: LineInfo.MessageBundle,
        at line: Int
    ) {
        guard
            let charRange = codeStorageDelegate.lineMap.lookup(line: line)?
                .range
        else { return }
        ...
        let messageView = StatefulMessageView.HostingView(
            messages: messageBundle.messages.sorted { $0.0 < $1.0 },
            theme: messageTheme,
        ...
        let principalCategory = messagesByCategory(
            messageBundle.messages.map(\.1)
        )[0].key
        let colour = messageTheme(principalCategory).colour
        let backgroundView = CodeBackgroundHighlightView(
            color: colour.withAlphaComponent(0.1)
        )
        let telescope: Int? =
            if messageBundle.messages.count == 1 {
                messageBundle.messages[0].1.telescope
            } else { nil }
    ...
    ///
    fileprivate func invalidateMessageViews(
        withIDs ids: [LineInfo.MessageBundle.ID]? = nil
    ) {
    ...
    /// IDs that do not have an associated message view cause no harm.
    ///
    fileprivate func removeMessageViews(
        withIDs ids: [LineInfo.MessageBundle.ID]? = nil
    ) {
    ...
    func process(event: LanguageServiceEvent) {
        ...
        case .tokensAvailable(let lineRange):
            codeStorageDelegate.requestSemanticTokens(
                for: lineRange,
                in: codeStorage
            )
        }
...
final class CodeContainer: NSTextContainer {
    ...
    override func lineFragmentRect(
        ...
        -> CGRect
    {
        let superRect = super.lineFragmentRect(
            forProposedRect: proposedRect,
            at: characterIndex,
            writingDirection: baseWritingDirection,
            remaining: remainingRect
        )
        let calculatedRect = CGRect(
            x: 0,
            y: superRect.minY,
            width: size.width,
            ...
            calculatedRect.width > 2 * MessageView.minimumInlineWidth
        {
            ...
            codeView.messageViews[messageBundleId]?.characterIndex =
                characterIndex
            codeView.messageViews[messageBundleId]?.lineFragementRect =
                calculatedRect
            ...
            {
                codeView.messageViews[messageBundleId]?
                    .characterIndexTelescope = oneLine.range.max
            }
            ...
            return CGRect(
                origin: calculatedRect.origin,
                size: CGSize(
                    width: calculatedRect.width
                        - MessageView.minimumInlineWidth,
...
func selectionDidChange<TV: TextView>(_ textView: TV) {
    ...
    if let location = textView.insertionPoint,
        let matchingBracketRange = codeStorage.matchingBracket(
            at: location,
            in: visibleLines
        )
    {
