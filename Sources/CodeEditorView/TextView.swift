...
extension TextView {
    ...
    var optLanguageService: LanguageService? {
        return (optCodeStorage?.delegate as? CodeStorageDelegate)?
            .languageService
    }
    ...
    var documentVisibleLines: Range<Int>? {
        guard let textLayoutManager = optTextLayoutManager,
            ...
            let lineMap = (optCodeStorage?.delegate as? CodeStorageDelegate)?
                .lineMap,
            lineMap.lines.count > 1  // this ensure that the line map has been initialised
        else { return nil }

        if let textRange = textLayoutManager.textViewportLayoutController
            .viewportRange
        {
            return lineMap.linesOf(
                range: textContentStorage.range(for: textRange)
            )
        } else {
            return nil
        }
    ...
    func fragmentBoundingRect(for range: NSRange? = nil) -> CGRect? {
        ...
        let textRange =
            ...
            { textRange } else { textContentStorage.documentRange }
        let rect = textLayoutManager.textLayoutFragmentBoundingRect(
            for: textRange
        )
    ...
    func invalidateBackground(forLineContaining textLocation: NSTextLocation) {
        invalidateBackground(
            forLinesContaining: NSTextRange(location: textLocation)
        )
    }
    ...
    func drawBackgroundHighlight(
        ...
        } else if let previousLocation = optTextContentStorage?.location(
            textLocation,
            offsetBy: -1
        ),
    ...
    private let highlightingAttributes = [
    ...
    extension UITextView: TextView {
        ...
        var insertionPoint: Int? {
            selectedRange.length == 0 ? selectedRange.location : nil
        }
        ...
        var selectedLines: Set<Int> {
            ...
            return Set(
                codeStorageDelegate.lineMap.linesContaining(
                    range: selectedRange
                )
            )
        }
        ...
        func showFindIndicator(for range: NSRange) {
            guard let textLayoutManager = optTextLayoutManager,
                let textContentStorage = textLayoutManager.textContentManager
                    as? NSTextContentStorage,
                let visibleTextRange = textLayoutManager
                    .textViewportLayoutController.viewportRange
            else { return }

            // Determine the visible portion of the range
            let visibleCharRange = textContentStorage.range(
                for: visibleTextRange
            )
            let visibleRange = NSIntersectionRange(visibleCharRange, range)

            // Set up a label view to animate as the indicator view
            guard
                let visibleTextRange = textContentStorage.textRange(
                    for: visibleRange
                ),
                let rect = textLayoutManager.boundingRectOfFirstTextSegment(
                    for: visibleTextRange
                )
            else { return }
            let label = UILabel(
                frame: rect.offsetBy(dx: textContainerOrigin.x, dy: 0)
            )
    ...
    extension NSTextView: TextView {
        ...
        var selectedLines: Set<Int> {
            ...
            let lineRanges: [Range<Int>] = selectedRanges.map { range in
                if let range = range as? NSRange {
                    return codeStorageDelegate.lineMap.linesContaining(
                        range: range
                    )
                } else {
                    return 0..<0
        ...
        var documentVisibleRect: CGRect {
            enclosingScrollView?.documentVisibleRect ?? bounds
        }
