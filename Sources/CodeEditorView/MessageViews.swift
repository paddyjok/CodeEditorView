...
extension Message {
    ...
    ///
    static func defaultTheme(for category: Message.Category) -> (
        colour: OSColor, icon: Image
    ) {
        switch category {
        case .live:
            return (
                colour: OSColor.green,
...
struct MessageInlineView: View {
    ...
    var body: some View {
        ...
        GeometryReader { geometryProxy in
            ...
            let colour =
                if invalidated { Color(OSColor.gray) } else {
                    Color(theme(categories[0]).colour)
                }
            ...
            HStack {
                ...
                HStack(alignment: .center, spacing: 0) {
                    ...
                    HStack {
                        Text(
                            messages.filter { $0.category == categories[0] }
                                .first?.summary ?? ""
                        )
            ...
            HStack(spacing: 0) {
                ...
                VStack(alignment: .leading, spacing: 6) {
                    ForEach(0..<messages.count, id: \.self) { i in
                        ...
                        if let description = messages[i].description {
                            Text(description)
                        }
                    }
                }
                ...
                .background(
                    GeometryReader { proxy in  // Propagate current width up the view tree
...
struct MessageView: View {
    ...
    var body: some View {
        ...
        ZStack(alignment: .topTrailing) {
            ...
            .frame(
                minWidth: MessageView.minimumInlineWidth,
...
extension StatefulMessageView {
    ...
    class HostingView: OSView {
        ...
        ///
        private let unfoldedState = StatefulMessageView.ObservableBool(
            bool: false
        )
        ...
        init(
            ...
            hostingView = OSHostingView(
                rootView: StatefulMessageView(
                    messages: messages.map(\.1),
                    theme: theme,
                    geometry: geometry,
                    background: background,
                    fontSize: fontSize,
...
let message3 = Message(
    ...
    description: AttributedString(
        "This doesn't seem right and also totally unclear "
...
struct MessageViews_Previews: PreviewProvider {
    ...
    static var previews: some View {
        ...
        MessagePopupView(
            messages: [message1, message5, message2, message4, message3],
            theme: Message.defaultTheme,
            invalidated: false
        )
        .frame(maxWidth: 320, minHeight: 15)
