...
private let logger = Logger(
    ...
    extension CodeView {
        ...
        func show(infoPopover: InfoPopover, for range: NSRange) {
            ...
            let nonEmptyScreenRect = NSRect(
                origin: screenRect.origin,
                size: CGSize(width: 1, height: 1)
            )
            let windowRect = window!.convertFromScreen(nonEmptyScreenRect)

            infoPopover.show(
                relativeTo: convert(windowRect, from: nil),
                of: self,
                preferredEdge: .maxY
            )
        }
        ...
        func infoAction() {
            ...
            Task {
                do {
                    if let info = try await languageService.info(
                        at: range.location
                    ) {

                        show(
                            infoPopover: InfoPopover(
                                displaying: info.view,
                                width: width
                            ),
                            for: info.anchor ?? range
                        )

                    }
                } catch let error {
                    logger.trace(
                        "Info action failed: \(error.localizedDescription)"
                    )
                }
            }
        }
    ...
    final class CompletionPanel: NSPanel {
        ...
        struct CompletionView: View {
            ...
            var completionsList: some View {
                ...
                if completions.items.isEmpty {
                    ...
                    List(completions.items, selection: $selection.selection) {
                        item in
                ...
                Group {
                    if completions.items.isEmpty {
                        ...
                        VStack(alignment: .leading, spacing: 0) {
                            ...
                            if let selectedCompletion =
                                (completions.items.first {
                                    $0.id == selection.selection
                                })
                            {
                                ScrollView {
                                    HStack {
                                        AnyView(
                                            selectedCompletion.documentationView
                                        )
                                        Spacer()
                                    }
        ...
        class HostedCompletionView: NSHostingView<CompletionView> {
            ...
            override func keyDown(with event: NSEvent) {
                guard let window = window as? CompletionPanel else {
                    ...
                    || event.keyCode == keyCodeUpArrow
                {
                    ...
                    if let selectedCompletion =
                        (window.completions.items.first {
                            $0.id == window.selection.selection
                        })
                    {
                        ...
                        window.progressHandler?(
                            .completion(
                                ...
                                selectedCompletion.insertRange
                            )
                        )
                ...
                } else if !event.modifierFlags.intersection([
        ...
        init() {
            hostingView = HostedCompletionView(
                rootView: CompletionView(
        ...
        func set(
            ...
            selection.selection =
                if let selected = (completions.items.first { $0.selected }) {
                    selected.id
            ...
            if completions.items.isEmpty && !explicitTrigger {
                ...
                hostingView.rootView = CompletionView(
                ...
                Task { @MainActor in
                    ...
                    contentView = HostedCompletionView(
                        rootView: CompletionView(
                            completions: self.completions,
                            selection: selection
                        )
    ...
    #Preview {
        @Previewable @StateObject var selection =
            CompletionPanel.ObservableSelection()
        let completions = Completions(
            isIncomplete: false,
            items: [
        ...
        CompletionPanel.CompletionView(
            completions: completions,
            selection: selection
        )
    }
    ...
    extension CodeView {
        ...
        func show(
            ...
            completionPanel.set(
                ...
                case .completion(let insertText, let insertRange):
                    ...
                    self?.insertText(
                        insertText,
                        replacementRange: insertRange ?? range
                    )
                    self?.completionPanel.progressHandler = nil
                    self?.completionPanel.close()
        ...
        ///   - explicitTrigger: The completion computation was explicitly triggered.
        ///
        func computeAndShowCompletions(at location: Int, explicitTrigger: Bool)
            async throws
        {
            guard let languageService = optLanguageService else { return }
            ...
            do {
                ...
                let reason: CompletionTriggerReason =
                    if completionPanel.isKeyWindow { .incomplete } else {
                        .standard
                    }
                let completions = try await languageService.completions(
                    at: location,
                    reason: reason
                )
                ...
                show(
                    completions: completions,
                    for: rangeForUserCompletion,
                    explicitTrigger: explicitTrigger
                )

            } catch let error {
                logger.trace(
                    "Completion action failed: \(error.localizedDescription)"
                )
            }
        }
        ...
        func completionAction() {
            ...
            if completionPanel.isKeyWindow {
                ...
                if let codeStorageDelegate = optCodeStorage?.delegate
                    as? CodeStorageDelegate,
                    !codeStorageDelegate.lineMap.isWithinComment(
                        range: NSRange(location: location, length: 0)
                    )
                {
                    completionTask = Task {
                        try await computeAndShowCompletions(
                            at: location,
        ...
        func considerCompletionFor(range: NSRange) {
            ...
            func rangeContentsWarrantsAutoCompletion() -> Bool {
                ...
                let atLeastOneLetter =
                    substring.unicodeScalars.first {
                        CharacterSet.letters.contains($0)
                    } != nil

                let notInMiddleOfIndentifier =
                    if let next = codeStorage.string[
                        NSRange(location: range.max, length: 1)
                    ],
            ...
            guard
                let codeStorageDelegate = optCodeStorage?.delegate
                    as? CodeStorageDelegate
            else { return }
            ...
            let withinComment = codeStorageDelegate.lineMap.isWithinComment(
                range: NSRange(location: range.max, length: 0)
            )
            if range.length > 0 && !withinComment
                ...
                && rangeContentsWarrantsAutoCompletion()
            {
                ...
                completionTask = Task {
                    ...
                    if range.length < 3 && !completionPanel.isKeyWindow {
                        try await Task.sleep(until: .now + .seconds(0.2))
                    }
