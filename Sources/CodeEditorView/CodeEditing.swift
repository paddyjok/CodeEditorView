...
extension CodeView: CodeEditorActions {
    ...
    @objc public func shiftLeft(_ sender: Any?) {
        shiftLeftOrRight(doShiftLeft: true)
    }
    @objc public func shiftRight(_ sender: Any?) {
        shiftLeftOrRight(doShiftLeft: false)
    }
            ...
            if location <= range.location {
                ...
                if max >= range.max {
                    ...
                    if range.length - overlap.length < -delta {
                        ...
                        return shifted(
                            endBy: -delta - (range.length - overlap.length)
...
extension CodeEditor.IndentationConfiguration {
    ...
    func currentIndentation(in line: any StringProtocol) -> Int {
        ...
        let index =
            (line.firstIndex { !($0 == " " || $0 == "\t") }) ?? line.endIndex
...
extension CodeView {
    ...
    func shiftLeftOrRight(doShiftLeft: Bool) {
        guard let textContentStorage = optTextContentStorage,
            ...
            let codeStorageDelegate = codeStorage.delegate
            ...
            if doShiftLeft {
                ...
                while remainingIndent > 0 {
                    ...
                    if character == " " {
                        ...
                        let tabIndent =
                            if length % tabWidth == 0 { tabWidth } else {
                                tabWidth - length % tabWidth
                            }
                        if tabIndent > remainingIndent {
                            ...
                            reminder += String(
                                repeating: " ",
                                count: tabIndent - remainingIndent
                            )
                ...
                let replacementRange = NSRange(
                    location: theLine.range.location,
                    length: length
                )
                codeStorage.replaceCharacters(
                    in: replacementRange,
                    with: reminder
                )
                return range.adjustSelection(
                    forReplacing: replacementRange,
                    by: reminder.utf16.count
                )
                ...
                codeStorage.replaceCharacters(
                    in: replacementRange,
                    with: indentation.defaultIndentation
                )
                return range.adjustSelection(
                    forReplacing: replacementRange,
                    by: 2
        ...
        textContentStorage.performEditingTransaction {
            processSelectedRanges { range in
                ...
                let lines = codeStorageDelegate.lineMap.linesContaining(
                    range: range
                )
                var newRange = range
                for line in lines {
    ...
    func comment() {
        ...
        func comment(line: Int, with range: NSRange) -> NSRange {
            ...
            codeStorage.replaceCharacters(
                in: replacementRange,
                with: singleLineCommentString
            )
            return range.adjustSelection(
                forReplacing: replacementRange,
                by: singleLineCommentString.count
            )
        }
        ...
        func uncomment(line: Int, with range: NSRange) -> NSRange {
            ...
            if let firstToken = theLine.info?.tokens.first,
                ...
                let tokenRange = firstToken.range.shifted(
                    by: theLine.range.location
                )
            {
        ...
        func isCommentBracketed(range: NSRange, on line: Int) -> NSRange? {
            ...
            for commentRange in commentRanges {
                if commentRange.intersection(localRange) == localRange
                    && (tokens.contains {
                        $0.range.location == commentRange.location
                            && $0.token == .nestedCommentOpen
                    })
                {
        ...
        func commentBracket(range: NSRange) -> NSRange {
            guard let (openString, closeString) = language.nestedComment else {
                return range
            }

            codeStorage.replaceCharacters(
                in: NSRange(location: range.max, length: 0),
                with: closeString
            )
            codeStorage.replaceCharacters(
        ...
        func uncommentBracket(range: NSRange, in commentRange: NSRange)
            -> NSRange
        {
            guard let (openString, closeString) = language.nestedComment else {
                return range
            ...
            codeStorage.deleteCharacters(
                in: NSRange(
                    location: commentRange.max - closeString.count,
                    length: closeString.count
                )
            )
            codeStorage.deleteCharacters(
                in: NSRange(
                    location: commentRange.location,
                    length: openString.count
                )
            )
            let newCommentRange =
                commentRange.shifted(by: -openString.count)?.shifted(
                    endBy: -closeString.count
                ) ?? commentRange
            return (range.shifted(by: -openString.count) ?? range).intersection(
                newCommentRange
            ) ?? range
        }
        ...
        textContentStorage.performEditingTransaction {
            processSelectedRanges { range in
                ...
                if range.length == 0 {
                    ...
                    guard
                        let theLine = codeStorageDelegate.lineMap.lookup(
                            line: firstLine
                        )
                    else { return range }
                    let lineLocation = range.location - theLine.range.location
                    if let commentRange = isCommentBracketed(
                        range: range,
                        on: firstLine
                    ) {
                        newRange = uncommentBracket(
                            range: range,
                            in: commentRange
                        )
                    } else {

                        let partiallyCommented = theLine.info?.commentRanges
                            .contains {
                                $0.contains(lineLocation)
                    ...
                    if isCommented(line: firstLine)
                        && isCommented(line: lastLine)
                    {
                        for line in lines.reversed() {
                            newRange = uncomment(line: line, with: newRange)
                        }
                    } else {
                        for line in lines.reversed() {
                            newRange = comment(line: line, with: newRange)
                        }
                    }
        ...
        textContentStorage.performEditingTransaction {
            processSelectedRanges { range in
                ...
                if range.length == 0 {
                    ...
                    guard
                        let line = codeStorageDelegate.lineMap.lineOf(
                            index: range.location
                        ),
                        let lineRange = codeStorageDelegate.lineMap.lookup(
                            line: line
                        )?.range
                    else { return range }
                    let _ = duplicate(range: lineRange)
                    return NSRange(
                        location: range.location + lineRange.length,
                        length: 0
                    )
    ...
    private func reindent(range: NSRange) -> NSRange {
        ...
        func currentIndentation(of line: Int) -> Int? {
            ...
            return indentation.currentIndentation(
                in: codeStorage.string[textRange]
            )
        }
        ...
        func startOfText(of line: Int) -> Int? {
            ...
            return indentation.startOfText(in: codeStorage.string[textRange])
                ?? 0
        }
        ...
        func predictedIndentation(for line: Int) -> Int {
            if language.indentationSensitiveScoping {
                ...
                while scannedLine >= 0 {
                    ...
                    if let index = startOfText(of: scannedLine) {
                        return index
                    } else {
                        scannedLine -= 1
                    }
                ...
                guard
                    let lineInfo = codeStorageDelegate.lineMap.lookup(
                        line: line
                    )
                else { return 0 }
                return (lineInfo.info?.curlyBracketDepthStart ?? 0)
        ...
        if range.length == 0 {
            ...
            codeStorage.replaceCharacters(
                in: NSRange(
                    ...
                    length: currentIndent
                ),
                with: indentString
            )
            return NSRange(
                location: lineInfo.range.location + indentString.count,
                length: 0
            )
            ...
            for line in lines {
                ...
                newRange = newRange.adjustSelection(
                    forReplacing: replacementRange,
                    by: indentString.count
                )
    ...
    @objc func insertTab() {
        ...
        func insertTab(in range: NSRange) -> NSRange {
            guard
                let firstLine = codeStorageDelegate.lineMap.lineOf(
                    index: range.location
                ),
                let lineInfo = codeStorageDelegate.lineMap.lookup(
                    line: firstLine
            ...
            let nextTabStopIndex =
                (column / indentation.tabWidth + 1) * indentation.tabWidth
            let replacementString =
                if indentation.preference == .preferTabs { "\t" } else {
                    String(repeating: " ", count: nextTabStopIndex - column)
                }
            codeStorage.replaceCharacters(in: range, with: replacementString)

            return NSRange(
                location: range.location + replacementString.utf16.count,
                length: 0
            )
        }
            ...
            textContentStorage.performEditingTransaction {
                processSelectedRanges { range in
                    ...
                    if range.length > 0 {
                        ...
                        let newRange =
                            if range.location - lineInfo.range.location
                                < indentDepth
                            { reindent(range: range) } else {
                                insertTab(in: range)
                            }
                        return newRange
    ...
    func insertReturn() {
        ...
        func predictedIndentation(after index: Int) -> Int {
            ...
            if language.indentationSensitiveScoping {
                ...
                let range = NSRange(
                    location: lineInfo.range.lowerBound,
                    length: index - lineInfo.range.lowerBound
                )
                guard
                ...
                let indentString = codeStorage.string[stringRange].prefix(
                    while: { $0 == " " || $0 == "\t" })
                ...
                let initialTokens = info.tokens.prefix {
                    $0.range.lowerBound < columnIndex
                }
                let openCurlyBrackets = initialTokens.reduce(0) {
                    $0
                        + ($1.token
                            == LanguageConfiguration.Token.curlyBracketOpen
                            ? 1 : 0)
                }
                let closeCurlyBrackets = initialTokens.reduce(0) {
                    $0
                        + ($1.token
                            == LanguageConfiguration.Token.curlyBracketClose
                            ? 1 : 0)
                }
                return
                    (curlyBracketDepth + openCurlyBrackets - closeCurlyBrackets)
                    * indentation.indentWidth
        ...
        textContentStorage.performEditingTransaction {
            processSelectedRanges { range in
                ...
                let desiredIndent =
                    if indentation.indentOnReturn {
                        predictedIndentation(after: range.location)
                    } else { 0 }
                let indentString = indentation.indentation(for: desiredIndent)
                codeStorage.replaceCharacters(
                    in: range,
                ...
                return NSRange(
                    location: range.location + 1 + indentString.count,
                    length: 0
                )
    ...
    func processSelectedRanges(with block: (NSRange) -> NSRange) {
        ...
        for rangeAsValue in ranges {
            ...
            if !newSelected.isEmpty {
                selectedRanges = newSelected.map { NSValue(range: $0) }
            }
        #elseif os(iOS) || os(visionOS)
