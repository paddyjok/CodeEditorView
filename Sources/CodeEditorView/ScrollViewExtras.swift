    ...
    extension UIScrollView {
        ...
        var verticalScrollPosition: CGFloat {
            ...
            set {
                let newOffset = max(
                    0,
                    min(newValue, bounds.height - contentSize.height)
                )
                if abs(newOffset - contentOffset.y) > 0.0001 {
                    setContentOffset(
                        CGPoint(x: contentOffset.x, y: newOffset),
                        animated: false
                    )
                }
            }
    ...
    extension NSScrollView {
        ...
        var verticalScrollPosition: CGFloat {
            ...
            set {
                ...
                (documentView as? CodeView)?.textLayoutManager?
                ...
                let newOffset = max(
                    ...
                    min(
                        newValue,
                        (documentView?.bounds.height ?? 0) - contentSize.height
                    )
                )
