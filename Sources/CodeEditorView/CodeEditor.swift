...
public struct CodeEditor {
    ...
    @Environment(\.codeEditorLayoutConfiguration) private
        var layoutConfiguration
    @Environment(\.codeEditorIndentationConfiguration) private
        var indentationConfiguration
    ...
    var definitiveLayout: LayoutConfiguration {
        return layout ?? layoutConfiguration
    }
    var definitiveSetActions: SetActions {
        return setActionsParam ?? setActions
    }
    ...
    @available(
        *,
        deprecated,
        message:
            "Use environment values for 'layout', 'breakUndoCoalescing', 'setActions', and 'setInfo"
    )
    public init(
        text: Binding<String>,
...
extension EnvironmentValues {
    ...
    @Entry public var codeEditorLayoutConfiguration:
        CodeEditor.LayoutConfiguration = .standard
}
...
extension CodeEditor {
    ...
    public struct IndentationConfiguration: Equatable, RawRepresentable {
        ...
        public static let standard = IndentationConfiguration(
...
extension EnvironmentValues {
    ...
    @Entry public var codeEditorIndentationConfiguration:
        CodeEditor.IndentationConfiguration = .standard
}
...
extension CodeEditor {
    ...
    public struct Info {
            ...
            init(selections: [NSRange], with lineMap: LineMap<LineInfo>) {
                    ...
                    let oneLine = lineMap.lookup(line: line)
                {
                    if range.length == 0 {
                        self = .insertionPoint(
                            line + 1,
                            range.location - oneLine.range.location + 1
                        )
                    } else {

                        let lastLine =
                            lineMap.lineOf(index: range.upperBound)
                            ?? lineMap.lines.count
                        if line == lastLine {
        ...
        public init(
            ...
            selectionSummary: SelectionSummary = .insertionPoint(1, 1)
        ) {
            self.language = language
...
extension EnvironmentValues {
    ...
    extension CodeEditor: UIViewRepresentable {
        ...
        public func makeUIView(context: Context) -> UITextView {
            ...
            // service.
            if let codeStorageDelegate = codeView.optCodeStorage?.delegate
                as? CodeStorageDelegate
            {
                codeStorageDelegate
                    .skipNextChangeNotificationToLanguageService = true
            }
        ...
        public func updateUIView(_ textView: UITextView, context: Context) {
            ...
            if text != codeView.text {  // Hoping for the string comparison fast path...
                ...
                if language.languageService
                    !== codeView.language.languageService
                {
                    (codeView.optCodeStorage?.delegate as? CodeStorageDelegate)?
                        .skipNextChangeNotificationToLanguageService = true
                }
                codeView.text = text
                ...
                as? CodeStorageDelegate
            {
                context.coordinator.info.selectionSummary =
                    Info.SelectionSummary(
                        selections: position.selections,
                        with: codeStorageDelegate.lineMap
                    )
            }
            if abs(
                position.verticalScrollPosition
                    - textView.verticalScrollPosition
            ) > 0.0001 {
                textView.verticalScrollPosition =
                    position.verticalScrollPosition
            }
            if theme.id != codeView.theme.id { codeView.theme = theme }
            if definitiveLayout != codeView.viewLayout {
                codeView.viewLayout = definitiveLayout
            }
            if indentationConfiguration != codeView.indentation {
                codeView.indentation = indentationConfiguration
            }
        ...
        public final class Coordinator: _Coordinator {
            ...
            func scrollPositionDidChange(_ scrollView: UIScrollView) {
                ...
                if abs(
                    position.verticalScrollPosition
                        - scrollView.verticalScrollPosition
                ) > 0.0001 {
                    position.verticalScrollPosition =
                        scrollView.verticalScrollPosition
                }
    ...
    extension CodeEditor: NSViewRepresentable {
        ...
        public func makeNSView(context: Context) -> NSScrollView {
            ...
            context.coordinator.breakUndoCoalescingCancellable =
                breakUndoCoalescing?.sink { [weak codeView] _ in
        ...
        public func updateNSView(_ scrollView: NSScrollView, context: Context) {
            guard let codeView = scrollView.documentView as? CodeView else {
                return
            }
            context.coordinator.updatingView = true
            defer {
            ...
            func selectionDidChange(_ textView: NSTextView) {
                ...
                if self.position.selections != newValue {
                    ...
                    if let codeStorageDelegate =
                        ((textView as? CodeView)?.optCodeStorage as? CodeStorage)?
                        .delegate
                        as? CodeStorageDelegate
                    {
                        self.info.selectionSummary = Info.SelectionSummary(
                            selections: newValue,
                            with: codeStorageDelegate.lineMap
                        )
                    }
...
extension CodeEditor.Position: RawRepresentable, Codable {
    ...
    public init?(rawValue: String) {
        ...
        if components.count == 2 {
            ...
            selections = components[0].components(separatedBy: ";").compactMap {
                parseNSRange(lexeme: $0)
            }
            verticalScrollPosition = CGFloat(Double(components[1]) ?? 0)

        } else {
            self = CodeEditor.Position()
        }
    }

    public var rawValue: String {
        let selectionsString = selections.map { "\($0.location):\($0.length)" }
            .joined(separator: ";")
        let verticalScrollPositionString = String(
            describing: verticalScrollPosition
        )
