...
extension LineMap<LineInfo> {
    ...
    func isWithinComment(range: NSRange) -> Bool {
        ...
        return commentRanges.contains {
            $0.intersection(lineRange) == lineRange
        }
    }
}
    ...
    func textStorage(_ textStorage: NSTextStorage, willProcessEditing editedMask: TextStorageEditActions, range editedRange: NSRange, changeInLength delta: Int) {
        ...
        func messageAffectedByEditFor(line: Int) -> Bool {
            ...
            if let (range, info) = lineMap.lookup(line: line) {
                info?.messages?.messages.allSatisfy {
                    $0.0 + $0.1.length < editedRange.location - range.location
                } ?? false
            } else {
                false
            }
...
//        var (affectedRange:highlightingRange, lines:highlightingLines) = tokenise(range: editedRange, in: textStorage)
        var highlightingRange = editedRange
        var highlightingLines = lines
...
//                    range: NSRange(
//                        location: editedRange.location + 1,
...
//                let extraHighlighting = tokenise(
//                    range: editedRange,
//                    in: textStorage
//                )
//                highlightingRange = highlightingRange.union(
//                    extraHighlighting.affectedRange
//                )
...
//            }
        ...
        tokenInvalidationLines = highlightingLines.count
...
extension CodeStorageDelegate {
    ...
    final class LineMapLocationService: LocationService {
        ...
        func textLocation(from location: Int) -> Result<TextLocation, Error> {
            guard let lineMap = codeStorageDelegate?.lineMap else {
                return .failure(ConversionError.lineMapUnavailable)
            }
                ...
                let oneLine = lineMap.lookup(line: line)
            {
                ...
                return .success(
                    TextLocation(
                        zeroBasedLine: line,
                        column: location - oneLine.range.location
                    )
                )

            } else {
                return .failure(ConversionError.locationOutOfBounds)
            }
        }
        ...
        func location(from textLocation: TextLocation) -> Result<Int, Error> {
            ...
            if let oneLine = lineMap.lookup(line: textLocation.zeroBasedLine) {
                ...
                return .success(
                    oneLine.range.location + textLocation.zeroBasedColumn
                )

            } else {
                return .failure(ConversionError.lineOutOfBounds)
            }
        }

        func length(of zeroBasedLine: Int) -> Int? {
            codeStorageDelegate?.lineMap.lookup(line: zeroBasedLine)?.range
                .length
        }
    }
    ...
    ///
    var lineMapLocationConverter: LineMapLocationService {
        LineMapLocationService(codeStorageDelegate: self)
    }
}
...
//                        lineInfo.commentRanges.append(
//                            NSRange(
//                                location: commentStart,
//                                length: lineRange.length - commentStart
//                            )
//                        )
...
//                        if commentDepth == 0 {
//                            lastCommentStart = token.range.location
...
//                continue
//            }
//            let thisLinesTokens = remainingTokens.prefix(while: {
//                $0.range.location < lineRange.max
//            })
...
//                let lineEntryRange = Range<String.Index>(
//                    lineEntry.range,
...
//                if let depth = lineEntry.info?.commentDepthStart,
//                    depth == commentDepth
//                {
...
//                    commentDepth > 0
//                    ? .tokenisingComment(commentDepth) : .tokenisingCode
//                let tokens =
//                    textStorage
...
//                highlightingRange = NSUnionRange(
//                    highlightingRange,
//                    lineEntry.range
//                )
//                highlightingLines += 1
...
//    }
    ...
    func requestSemanticTokens(
        ...
        Task {
            do {
                if let semanticTokens = try await languageService?.tokens(
                    for: lines
                ) {

                    guard lines.count == semanticTokens.count else {
                        logger.trace(
                            "Language service returned an array of incorrect length; expected \(lines.count), but got \(semanticTokens.count)"
                        )
                        return
                    }
                    ...
                    Task { @MainActor in
                        ...
                        for i in 0..<lines.count {
                            merge(
                                semanticTokens: semanticTokens[i],
                                into: firstLine + i
                            )
                        }

                        // Request redrawing for those lines
                        if let textStorageObserver = textStorage
                            .textStorageObserver
                        {
                            let range = lineMap.charRangeOf(lines: lines)
                            textStorageObserver.processEditing(
                                for: textStorage,
                                ...
                                invalidatedRange: NSRange(
                                    location: 0,
                                    length: textStorage.string.count
                                )
                            )
                ...
                logger.trace(
                    "Failed to get semantic tokens for line range \(lines): \(error.localizedDescription)"
                )
            }
        }
    }
    ...
    ///
    private func merge(
        semanticTokens: [(token: LanguageConfiguration.Token, range: NSRange)],
        into line: Int
    ) {
        ...
        for i in 0..<tokens.count {
                ...
                semanticToken.range.location <= token.range.location
            {
                ...
                if semanticToken.range == token.range
                    ...
                    && (semanticToken.token.isIdentifier
                        || semanticToken.token.isOperator
                        || semanticToken.token == .keyword)
                {
                    tokens[i] = LanguageConfiguration.Tokeniser.Token(
                        token: semanticToken.token,
                        range: token.range
                    )
                }
            }
...
extension CodeStorageDelegate {
    ...
    func tokenCompletion(for codeStorage: CodeStorage, at index: Int) -> Int {
        ...
        ///
        func matchingLexemeForOpeningBracket(
            _ token: LanguageConfiguration.Token
        ) -> String? {
            if token.isOpenBracket, let matching = token.matchingBracket,
                let lexeme = language.lexeme(of: matching)
            {
                return lexeme
            } else {
                return nil
        ...
        ///
        func overlapping(
            _ previousToken: LanguageConfiguration.Tokeniser.Token,
        ...
        // the previous token
        if let previousToken = previousTypedToken,
            ...
            !overlapping(previousToken, currentTypedToken)
        {
            ...
            // bracket
            if let matchingPreviousLexeme = matchingLexemeForOpeningBracket(
                previousToken.token
            ) {
                ...
                if let currentToken = currentTypedToken {
                    ...
                    } else if let matchingCurrentLexeme =
                        matchingLexemeForOpeningBracket(currentToken.token)
                    {

                        // The current token is another opening bracket => insert matching closing for the current and previous
                        // opening bracket
                        completingString =
                            matchingCurrentLexeme + matchingPreviousLexeme
            ...
            if let string = completingString {
                ...
                codeStorage.replaceCharacters(
                    in: NSRange(location: index + 1, length: 0),
                    with: string
                )
...
extension CodeStorageDelegate {
    ...
    func add(message: TextLocated<Message>) -> LineInfo.MessageBundle? {
        guard
            var info = lineMap.lookup(line: message.location.zeroBasedLine)?
                .info
        else { return nil }
    ...
    ///
    func messages(at line: Int) -> LineInfo.MessageBundle? {
        return lineMap.lookup(line: line)?.info?.messages
    }
